...
def draw_cover(c: canvas.Canvas, title: str, subtitle: str, label: str) -> None:
    ...
    c.setFont(SANS_BOLD, 10)
    c.setFillColor(colors.white)
    c.drawCentredString(PAGE_W / 2, 1.14 * inch, "PAMELLA GREAR")
    c.setFont(SANS, 8)
...
def cover_image(path: Path, title: str, subtitle: str, label: str) -> None:
    ...
    author_box = draw.textbbox((0, 0), "PAMELLA GREAR", font=sans_bold)
    draw.text(((width - (author_box[2] - author_box[0])) / 2, 2240), "PAMELLA GREAR", fill="#FFFFFF", font=sans_bold)
...
<metadata xmlns:dc="http://purl.org/dc/elements/1.1/">
  ...
  <dc:creator>Pamella Grear</dc:creator><dc:language>en</dc:language><dc:publisher>Pam Grear Publishing LLC</dc:publisher>

...
revoke all on table public.compassion_messages from anon, authenticated, service_role;
grant select, insert, update on table public.compassion_messages to service_role;
...
drop policy if exists "service role moderates compassion messages" on public.compassion_messages;
...
create policy "service role moderates compassion messages"
  on public.compassion_messages for update to service_role using (true) with check (true);
...
values
  ('The Compassion Hub', 'A note from us', 'You are not alone in this season. Take a breath, release what you cannot carry, and trust that good days are still ahead.', true, 'seed', 'welcome-karen', now() - interval '6 days', now()),
  ('The Compassion Hub', 'A note from us', 'May you remember that small steps still move you forward. Be gentle with yourself today.', true, 'seed', 'welcome-miguel', now() - interval '5 days', now()),
  ('The Compassion Hub', 'A note from us', 'You are deeply loved. Let that truth be the anchor that holds you steady no matter what comes.', true, 'seed', 'welcome-aisha', now() - interval '4 days', now()),
  ('The Compassion Hub', 'A note from us', 'Some days are harder than others. Keep choosing hope—one moment, one breath, one step at a time.', true, 'seed', 'welcome-james', now() - interval '3 days', now()),
  ('The Compassion Hub', 'A note from us', 'Kindness you show today may be the light someone else needs to keep going. Thank you for being that light.', true, 'seed', 'welcome-tanya', now() - interval '2 days', now()),
  ('The Compassion Hub', 'A note from us', 'There is purpose in your story. What you have been through can become a source of strength for someone else.', true, 'seed', 'welcome-derek', now() - interval '1 day', now())

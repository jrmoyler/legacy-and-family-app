...
This directory contains the complete app library: six books and two companion workbooks in PDF and EPUB, plus The Compassion Legacy Journal in illustrated PDF format and production cover art for all publications, standalone resources, and the home page.
...
`covers/` holds a full-size production cover for every title, with a 640px
grid thumbnail in `covers/thumbs/`. EPUB covers are extracted from their masters;
the PDF-only journal uses its committed illustrated cover. Both are built by
...
`SHA256SUMS.txt`, which stays a manifest of the 17 publication files;
...
For the complete release audit—catalog order, all 17 checksums and byte counts,
canonical author metadata, EPUB package structure, PDF parsing, publication and
product cover renditions, and the app's PDF/EPUB links—run:
